...
# 复制 .config 到编译输出目录
cp ../../immortalwrt-mt798x-6.6/.config ../../immortalwrt-mt798x-6.6/bin/targets/mediatek/filogic/

# 发布信息（发布说明由 RAX3000M-EMMCinit.sh 生成到 Releases.txt）
TAG="$(date +'%Y%m%d%H%M')-rax3000m-237"

# 进入输出目录后上传固件到 Releases
cd ../../immortalwrt-mt798x-6.6/bin/targets/mediatek/filogic
